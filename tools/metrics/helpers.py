...
def compute_fal(
    ...
    if "flags_computed_at" in df.columns and df["flags_computed_at"].notna().any():
        if "features_ready_ms" in df.columns and "speed_features_ready_at" in df.columns:
            features_ready_ms = pd.to_numeric(df["features_ready_ms"], errors="coerce")
            speed_ready = persisted_at_to_s(df["speed_features_ready_at"]) * 1000

            corrected = (features_ready_ms.notna()) & (speed_ready.notna()) & (features_ready_ms > speed_ready)

            if corrected.any():
                persist_source = "features_ready_ms"
                persist_is_ms = True
            elif df["speed_features_ready_at"].notna().any():
