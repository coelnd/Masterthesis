"""
Kappa architecture Flink job for real-time event processing with anomaly flags.
"""
...
logger.info("Initializing Flink execution environment for Kappa architecture")
...
try:
    t_env.execute_sql(machines_lookup_table)
    logger.info("Created JDBC lookup table for machines (Kappa Stream)")
except Exception as e:
...
CREATE TEMPORARY TABLE beacon_events (
    ...
    WATERMARK FOR kafka_ts_ltz AS kafka_ts_ltz - INTERVAL '2' YEAR
) WITH (
  'connector' = 'kafka',
  'topic' = '{TOPIC_NAME}',
  'properties.bootstrap.servers' = '{KAFKA_BROKER}',
  'properties.group.id' = 'flink-kappa-consumer',
...
try:
    ...
    logger.info("Created Kafka source table: beacon_events")
except Exception as e:
    logger.error("Failed to create source table: %s", e, exc_info=True)
    raise

dishwasher_sink_table = f"""
...
try:
    t_env.execute_sql(dishwasher_sink_table)
    logger.info("Created PostgreSQL dishwasher_data sink table")
except Exception as e:
...
SELECT
    COALESCE(e.Id, CONCAT('fallback_', CAST(EXTRACT(EPOCH FROM e.kafka_ts_ltz) * 1000 AS VARCHAR), '_', CAST(e.MACAddressString AS VARCHAR))) AS event_id,
    ...
    CAST(e.DateTimeUTC AS BIGINT) * 1000 AS event_time_utc_ms,
    ...
    CAST(NULL AS DOUBLE) AS latency_seconds,  -- Calculated by PostgreSQL trigger (standardized)
    ...
    COALESCE(
        w.window_end,
        e.kafka_ts_ltz + INTERVAL '20' MINUTE
    ) AS flags_computed_at,
    COALESCE(
        w.window_end,
        e.kafka_ts_ltz + INTERVAL '20' MINUTE
    ) AS stream_features_ready_at
FROM beacon_events AS e
LEFT JOIN (
    SELECT
        MACAddressString,
        TUMBLE_START(kafka_ts_ltz, INTERVAL '20' MINUTE) AS window_start,
        TUMBLE_END(kafka_ts_ltz, INTERVAL '20' MINUTE) AS window_end,
        AVG(MeanConductivity) AS avg_conductivity,
        MAX(MeanWashTankTemp) AS max_wash_temp,
        MIN(MeanWashTankTemp) AS min_wash_temp,
        COUNT(CASE WHEN MeanWashTankTemp IS NOT NULL THEN 1 END) AS wash_temp_count,
        MAX(MeanRinseTemp) AS max_rinse_temp,
        MIN(MeanRinseTemp) AS min_rinse_temp,
        COUNT(CASE WHEN MeanRinseTemp IS NOT NULL THEN 1 END) AS rinse_temp_count,
        COUNT(*) AS total_events
    FROM beacon_events
    GROUP BY TUMBLE(kafka_ts_ltz, INTERVAL '20' MINUTE), MACAddressString
) AS w
ON e.MACAddressString = w.MACAddressString
   AND e.kafka_ts_ltz >= w.window_start
   AND e.kafka_ts_ltz < w.window_end
...
try:
    ...
    logger.info("Kappa Flink job submitted and executed successfully")
except Exception as e:
